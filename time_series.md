...
-   *var(*$y_{t}$*)=E*$\lbrack y_{t} - E(y_{t})\rbrack^{2}$

-   *cov*${(y}_{t},y_{t + k})$ *= cov*${(y}_{t + m},y_{t + m + k})$
...
$$Y_{t} = \alpha_{0} + \alpha_{1}Y_{t - 1} + \alpha_{2}Y_{t - 2} + \ldots + \alpha_{p}Y_{t - p} + \varepsilon_{t}$$
...
$$\Upsilon_{\tau} = \varepsilon_{t} - \theta_{1}\varepsilon_{t - 1} - \theta_{2}\varepsilon_{t - 2} - \ldots - \theta_{q}\varepsilon_{t - q}$$
...
ορίζουμε ως πρώτες διαφορές $y_{t}^{'} = y_{t} - y_{t - 1}$.
